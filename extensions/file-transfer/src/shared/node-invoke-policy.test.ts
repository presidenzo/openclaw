...
describe("file-transfer node invoke policy", () => {
  ...
  it("uses fresh approval for file.fetch canonical paths before requesting bytes", async () => {
    const policy = createFileTransferNodeInvokePolicy();
    const approvals = {
      request: vi.fn(async () => ({ id: "approval-1", decision: "allow-once" as const })),
    };
    ...
    expect(approvals.request).toHaveBeenCalledWith(
      expect.objectContaining({
        title: "Read file: /private/tmp/link.txt",
        toolName: "file.fetch",
      }),
    );
    expect(invokeNode).toHaveBeenCalledTimes(2);
    expect(invokeNode).toHaveBeenNthCalledWith(1, {
      params: expect.objectContaining({ path: "/tmp/link.txt", preflightOnly: true }),
    });
  ...
  it("uses fresh approval for file.write canonical paths before the mutating node call", async () => {
    const policy = createFileTransferNodeInvokePolicy();
    const approvals = {
      request: vi.fn(async () => ({ id: "approval-1", decision: "allow-once" as const })),
    };
    ...
    expect(approvals.request).toHaveBeenCalledWith(
      expect.objectContaining({
        title: "Write file: /private/tmp/out.txt",
        toolName: "file.write",
      }),
    );
    expect(invokeNode).toHaveBeenCalledTimes(2);
  ...
  it("uses fresh approval for each dir.fetch entry before requesting the archive", async () => {
    const policy = createFileTransferNodeInvokePolicy();
    const approvals = {
      request: vi.fn(async () => ({ id: "approval-1", decision: "allow-once" as const })),
    };
    const { ctx, invokeNode } = createCtx({
      ...
      pluginConfig: {
        nodes: {
          "node-1": {
            ask: "on-miss",
            allowReadPaths: ["/home/me", "/home/me/ok.txt"],
          },
        },
      },
      approvals,
    ...
    invokeNode
      .mockResolvedValueOnce({
        ...
        payload: {
          ok: true,
          path: "/home/me",
          entries: ["ok.txt", "needs-approval.txt"],
          fileCount: 2,
          preflightOnly: true,
        },
      })
    ...
    expect(approvals.request).toHaveBeenCalledWith(
      expect.objectContaining({
        title: "Fetch directory: /home/me/needs-approval.txt",
        toolName: "dir.fetch",
      }),
    );
    expect(invokeNode).toHaveBeenCalledTimes(2);
